...
def parse_args():
    ...
    parser.add_argument(
        ...
        '--checkpoint',
        type=str,
        default='/home/yw2875/gif-super-resolution/pxpp/params_cifar.ckpt',
...
def pixel_cnn(batch_size=16, image_size=32, seed=2702, checkpoint='/home/yw2875/gif-super-resolution/pxpp/params_cifar.ckpt', save_dir='/tmp'):
    g = tf.Graph()
    with g.as_default():
        print('Creating model')
        input_channels = 4  # 3 channels for RGB and 1 channel of all ones 
        image_size = (batch_size, image_size, image_size,
                      input_channels)

        row_input = tf.placeholder(
            tf.float32, [batch_size, 1, image_size, input_channels],
            name='row_input')
        pixel_input = tf.placeholder(
            tf.float32, [batch_size, 1, 1, input_channels],
            name='pixel_input')
        ...
        sample, fast_nn_out, v_stack = model_spec(
            row_input, pixel_input, row_id, col_id, image_size, seed=seed)
        ...
        output_images = np.zeros((batch_size, image_size, image_size, 3))
            ...
            for row in range(image_size):
                ...
                if row == 0:
                    x_row_input = np.zeros(
                        (batch_size, 1, image_size, input_channels))
                else:
                    x_row_input = output_images[:, (row - 1):row, :, :]
                    x_row_input = np.concatenate(
                        (x_row_input, np.ones(
                            (batch_size, 1, image_size, 1))),
                        axis=3)
                ...
                for col in range(image_size):
                    ...
                    if col == 0:
                        x_pixel_input = np.zeros(
                            (batch_size, 1, 1, input_channels))
                    else:
                        x_pixel_input = output_images[:, row:(row + 1),
                                                      (col - 1):col, :]
                        x_pixel_input = np.concatenate(
                            (x_pixel_input, np.ones((batch_size, 1, 1, 1))),
                            axis=3)
    ...
    #         plt.savefig(os.path.join(save_dir, 'images_%d.png' % batch))
...
if __name__=='__main__':
    ...
    pixel_cnn(batch_size, image_size, seed, checkpoint, save_dir)
